...
def pca():
    ...
    var2 = DoubleVar()
    L = Label(master, text= "Define point-size").grid(row=3,column=1)
...
def volcano():
    ...
    L = Label(master, text= "Apply SD filtering to remove data outliers").grid(row=4,column=1)
    Warning = Label(master, text= "Warning! Applying SD flitering may \n remove points from your dataset!",fg="red").grid(row=6,column=1)
    
    varSD = IntVar()
    varSD.set(1000)
    #L= Label(master, text="Choose legend location:").grid(row=8,column=1)
    R= Radiobutton(master, text="10 SD's from mean",variable=varSD, value=10).grid(row=4,column=2)
    R= Radiobutton(master, text="20 SD's from mean",variable=varSD, value=20).grid(row=5,column=2)
    R= Radiobutton(master, text="30 SD's from mean",variable=varSD, value=30).grid(row=6,column=2)
    R= Radiobutton(master, text="40 SD's from mean",variable=varSD, value=40).grid(row=7,column=2)
    R= Radiobutton(master, text="Do not filter outliers",variable=varSD, value=1000).grid(row=8,column=2)
    #sds = Scale(master, from_=10, to=50,resolution=5,orient=HORIZONTAL,variable=var3).grid(row=4,column=2)
    L = Label(master, text= "Define size of points").grid(row=9,column=1)
    points = Scale(master, from_=2, to=40,resolution=1,orient=HORIZONTAL,variable=var5).grid(row=9,column=2)
    #points.set(20)
    #points.grid(row=5,column=2)
    L = Label(master, text= "Define plot title").grid(row=10,column=1)
    E = Entry(master, textvariable=var4).grid(row=10,column=2)
    L = Label(master, text= 'Provide a list of select genes, separated by ","').grid(row=11,column=1)
    E = Entry(master, textvariable=var6).grid(row=11,column=2)

    #L= Label(master,text="Toggle legend location:").grid(row=8,column=1)

    var840 = IntVar()
    var840.set(2)
    L= Label(master, text="Choose legend location:").grid(row=12,column=1)
    R= Radiobutton(master, text="Top Left",variable=var840, value=1).grid(row=12,column=2)
    R= Radiobutton(master, text="Top Right",variable=var840, value=2).grid(row=13,column=2)
    R= Radiobutton(master, text="Bottom Left",variable=var840, value=3).grid(row=14,column=2)
    R= Radiobutton(master, text="Bottom Right",variable=var840, value=4).grid(row=15,column=2)
    R= Radiobutton(master, text="No Legend",variable=var840, value=0).grid(row=16,column=2)
    ...
    B = Button(master, text ="Update plot", command = applysliders).grid(row=17,column=1)
    B = Button(master, text ="Save plot", command = save_fig).grid(row=17,column=2)
    B = Button(master, text ="Return to Main Menu", command = restart_main).grid(row=18,column=2)
...
def heatmap():
    ...
    L = Label(master, text= "Define p-value for thresholding").grid(row=2,column=1)
...
def main_gui():
    ...
    class Example(Frame):
        ...
        def initUI(self):
            ...
            self.parent.title("PadPlot_Live")
            self.intro=Label(text= "Welcome to PadPlot! \n PadPlot accepts Deseq2 outputs combined with count-matrices to plot RNA-seq data \n\n Padplot does not: \n\n -Perform statistical analysis \n -Create count matrices \n -Align or assemble sequencing data \n").grid(row=0, column=0, padx=20, pady=5,columnspan=3)
            self.button_1=Button()
            self.button_1.grid(row=1, column=0, padx=20, pady=5,columnspan=3)
            self.button_1.configure(text='Choose an RNA-seq csv file to plot',command=self.import_csv_data)
            self.label_1=Label()
            self.label_1.grid(row=2, column=0, padx=20, pady=5,columnspan=3)
            self.label_1.configure(text='')

            self.button_2=Button()
            self.button_2.grid(row=3, column=0, padx=20, pady=5,columnspan=3)
            self.button_2.configure(text='Choose a folder to save plots to',command=self.get_figure_path)
            self.label_2=Label()
            self.label_2.grid(row=4, column=0, padx=20, pady=5,columnspan=3)
            self.label_2.configure(text='')

            var420 = IntVar()
            var420.set(2)
            self.grouplabel=Label(text="Choose number of Groups:").grid(row=5,column=0, padx=20, pady=5,columnspan=3)
            self.groupradio1=Radiobutton(text="Two",variable=var420, value=2).grid(row=6, padx=20, pady=5,column=0)
            self.groupradio2=Radiobutton(text="Three",variable=var420, value=3).grid(row=6, padx=20, pady=5,column=1)
            self.groupradio3=Radiobutton(text="Four",variable=var420, value=4).grid(row=6, padx=20, pady=5,column=2)

            self.plotlabel= Label(text="Please choose your plot").grid(row=8, column=0, padx=20, pady=5,columnspan=3)
            self.plotbutton1 = Button(text="PCA",command=self.sel_pca).grid(row=9, column=0, padx=20, pady=5)
            self.plotbutton2 = Button(text="Volcano",command=self.sel_volcano).grid(row=9, column=1, padx=20, pady=5)
            self.plotbutton3 = Button(text="Heat Map",command=self.sel_heatmap).grid(row=9, column=2, padx=20, pady=5)
            self.authorlabel= Label(text="PadPlot authored by SimonF92, contact me at s.fisher.1@research.gla.ac.uk").grid(row=15, column=0, padx=20, pady=5,columnspan=3)
            self.helplabel= Button(text='Help',command=self.debug_options).grid(row=34, column=0, padx=20, pady=5)
            self.cancel= Button(text='Cancel',command=self.exitout).grid(row=35, column=0, padx=20, pady=5)
        ...
        def import_csv_data(self):
            global csv_file_path
            csv_file_path = askopenfilename()      
            self.label_1.configure(text= csv_file_path)

        def get_figure_path(self):

            global fullpath
            fullpath= askdirectory()
            self.label_2.configure(text= fullpath)
        ...
        def sel_pca(self):
            ...
            sel=1
            self.master.destroy()
        def sel_volcano(self):
            global sel
            sel=2
            self.master.destroy()
        def sel_heatmap(self):
            global sel
            sel=3
            self.master.destroy()
