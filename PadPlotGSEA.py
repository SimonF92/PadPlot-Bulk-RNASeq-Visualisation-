...
def volcanoplot():
    ...
    option = st.sidebar.selectbox('Choose Volcano Type',('Standard', 'PadPlot Gene Ontology Labelling (Human Only)'))
    ...
    if option=='PadPlot Gene Ontology Labelling (Human Only)':
...
def geneset_enrichment():
    ...
    def fullgenesets():
        ...
        scatterplot=ax.scatter(y=fullset_head.index,x=fullset_head['neglogp'],c=fullset_head['nes'],cmap='seismic',edgecolors='black',s=70)
        ...
        plt.title('Enriched Pathways and Functions',size=15)
        plt.xlabel('FDR (-log)')
    ...
    if gseaplot:
        ...
        head=st.sidebar.slider('Number of Pathways', 0, 50, 1)
...
vol, heat, princ, viol, genesets = st.beta_columns(5)
...
gse=genesets.checkbox("PadPlot Gene Set Enchrichment (Human Only)")
